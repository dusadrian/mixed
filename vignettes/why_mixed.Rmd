...
While creating the `mixed_labelled` class to deal with the declared missing values, other interesting possibilities opened up. In particular, vectors containing tagged NA values gain a special class that allows formatting / printing such values:
...
* it also accepts regular numbers
...
The difference is particularly obvious when extracting the tag:


```{r}
haven::na_tag(tag(12345))

get_tag(tag(12345))
```

This was made possible because, as it turns out, it is possible to tag an NA value using not only one extra, but actually two extra bytes.
